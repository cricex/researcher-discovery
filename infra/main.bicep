...
@description('Optional Azure Container Registry name override. Must be globally unique, 5-50 lowercase alphanumeric chars.')
...
@description('The startup image for initial provisioning. azd deploy later replaces this image with your built app image.')
...
var appName = !empty(containerAppName)
  ? containerAppName
  : take('ca-${environmentName}-mcp-${take(resourceToken, 4)}', 32)
...
output SERVICE_MCP_NAME string = containerApp.name
output SERVICE_MCP_URI string = 'https://${containerApp.properties.configuration.ingress.fqdn}'
output SERVICE_MCP_CONTAINER_ENVIRONMENT_NAME string = containerAppsEnvironment.name
